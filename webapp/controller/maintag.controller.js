sap.ui.define([
	"com/shunyu/labelreplay/controller/BaseController",
	...
	"com/shunyu/labelreplay/Formatter",
	"sap/ui/core/syncStyleClass",
	"com/shunyu/labelreplay/LodopFuncs"

], function(BaseController, Controller, UIComponent, MessageBox, MessageToast, Fragment, Dialog, JSONModel, Filter,
	FilterOperator, ODataModel, Formatter, syncStyleClass) {
	"use strict";
	
	return BaseController.extend("com.shunyu.labelreplay.controller.maintag", {
		_tagfilterbar: null,
		
		// 初始化
		onInit: function() {
			// var rootPath = jQuery.sap.getModulePath("com.shunyu.labelreplay");
		...
		HUHandleValueHelp: function () {
			Fragment.load({
					name: "com.shunyu.labelreplay.view.HUValueHelp",
					controller: this
		...
		materialHandleValueHelp: function () {
			Fragment.load({
					name: "com.shunyu.labelreplay.view.MaterialValueHelp",
					controller: this
				}).then(function (oValueHelpDialog) {
					this._oValueHelpDialog = oValueHelpDialog;
					this.getView().addDependent(this._oValueHelpDialog);
		...
		customerHandleValueHelp: function () {
			Fragment.load({
					name: "com.shunyu.labelreplay.view.CustomerValueHelp",
					controller: this
				}).then(function (oValueHelpDialog) {
					this._oValueHelpDialog = oValueHelpDialog;
					this.getView().addDependent(this._oValueHelpDialog);
		...
		labelHandleValueHelp: function () {
			Fragment.load({
					name: "com.shunyu.labelreplay.view.LabelValueHelp",
					controller: this
				}).then(function (oValueHelpDialog) {
					this._oValueHelpDialog = oValueHelpDialog;
					this.getView().addDependent(this._oValueHelpDialog);
		...
		printerHandleValueHelp: function () {
			Fragment.load({
					name: "com.shunyu.labelreplay.view.PrinterValueHelp",
					controller: this
				}).then(function (oValueHelpDialog) {
					this._oValueHelpDialog = oValueHelpDialog;
					this.getView().addDependent(this._oValueHelpDialog);
		...
		printLabelTag: function() {
			...
			if (oTable.length) {
				...
				if (!this.byId("printLabelTag")) {
					Fragment.load({
						id: oView.getId(),
						name: "com.shunyu.labelreplay.view.newtagDialog",
						controller: this
		...
		print: function() {
			...
			if (date !== "" || date.length > 0) {
				if (!date.match(/^(?:(?!0000)[0-9]{4}([-/.]?)(?:(?:0?[1-9]|1[0-2])([-/.]?)(?:0?[1-9]|1[0-9]|2[0-8])|(?:0?[13-9]|1[0-2])([-/.]?)(?:29|30)|(?:0?[13578]|1[02])([-/.]?)31)|(?:[0-9]{2}(?:0[48]|[2468][048]|[13579][26])|(?:0[48]|[2468][048]|[13579][26])00)([-/.]?)0?2([-/.]?)29)$/)) 
				{
